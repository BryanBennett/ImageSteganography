...
int process_image_args(int argc, char *argv[], int* hide_ptr, int* bit_ptr, char image_file_name[], char text_file_name[]) {
    ...
    if ( argc < 6) {
        printf("Insufficient quantity of arguments provided via command line\n");
    ...
    if(!(hide ^ recover) || !got_bit || optind >= argc){
        print_command_usage(argv[0]);
        if ( !(hide ^ recover) ) {
            printf("-h or -r not specified. Breaking\n");
        }
        if ( !got_bit ) {
            printf("Bit argument must be either 1, 2, or 4. Breaking\n");
        }
        return 0;
    }
